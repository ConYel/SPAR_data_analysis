...
path <- "/home/"
setwd(path)
smallRNA_files <- dir(path,pattern = "smRNA_gene_expression.xls", recursive = TRUE)
...
dt %>% write_tsv(str_glue("{writting_path}results_matrix_{date}_SPAR.txt"))
```
...
group <- as_factor(str_glue("{targets_file$total_region}_{targets_file$tissue}")) %>% str_replace(" ","_")
...
x1$col_<- inner_join(x1$samples,pal,by= "group")$value
...
for (i in seq_along(l_hc)) {
  myplclust(l_hc[[i]],
            lab.col = x1$col_date$value, #x1$colours,pal1$value
  ...
  legend("topright",
       legend = levels(x1$col_date$date_seq),
       fill = levels(as_factor(x1$col_date$value)), #x1$colours  
       bty="n",
       cex = 0.9)
}
