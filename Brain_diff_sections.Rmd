...
norm_list[["TMM"]] %>% 
  ...
  write_tsv(str_glue("{dat_path}/CPM_brain_testis_piR_{date}_{my_exp}.txt"))
...
pdf(str_glue("{dat_path}/Heatmap_brain_testis_{date}_{my_exp}.pdf"))
Heatmap(scaled_mat,
        top_annotation = ha,
        col=f,
        show_row_dend = TRUE,
...
samples <- samples[1:6,] %>% droplevels()
...
list_compar <- list(Thalamus = c("Thalamus","Total"),
                    Frontal_Lobe = c("Frontal Lobe","Total"),
                    Occipital_Lobe = c("Occipital Lobe","Total"),
                    Parietal_Lobe = c("Parietal Lobe","Total"),
                    Temporal_Lobe = c("Temporal Lobe","Total"))
...
complete_DEs <- rbindlist(mynoiseDE2,use.names = TRUE,
                          idcol = "comparison",fill = TRUE) %>%
  as_tibble() %>% 
  write_tsv(str_glue("{dat_path}/DE_NOISeq_vs_Total_{date}_{my_exp}.txt"))
