...
EDASeq::plotRLE(dgl_NOIs$counts, col=dgl_NOIs$colours, 
                outline=FALSE, las=3, ylab="Relative Log Expression", 
                cex.axis=1, cex.lab=1)
EDASeq::plotPCA(dgl_NOIs$counts, 
                col=dgl_NOIs$colours, cex=1, cex.axis=1, cex.lab=1)
...
```{r}
# not normalized
cpm_unorm <- cpm(dgl_NOIs,log=F,prior.count=5)
lcpm_unorm <- cpm(dgl_NOIs,log=TRUE,prior.count=5)
# EDA norm ----
...
edger_norm <- lapply(norm_method, function(x){
  calcNormFactors(dgl_NOIs, method = x)
})
...
sapply(seq_along(norm_list) ,function(x, n, i){ 
       EDASeq::plotRLE(x[[i]], col=dgl_NOIs$colours,
                       outline=FALSE, las=3, 
                       ylab="Relative Log Expression", 
                       cex.axis=1, cex.lab=1, main = str_glue("{n[[i]]}"));
  legend("topright",
       legend= levels(as_factor(dgl_NOIs$samples$group)),
       fill = levels(as_factor(dgl_NOIs$colours)),
       bty="n",
       cex = 1.5, inset = c(.01,.01) )}, x = norm_list, n = names(norm_list))
dev.off()


#cpm_TMM <- cpm(x1_TMM,normalized.lib.sizes = TRUE, prior.count=5)

#EDASeq::plotRLE(cpm_TMM, col=dgl_NOIs$colours, 
#                outline=FALSE, las=3, ylab="Relative Log Expression", 
...
for (i in seq_along(l_hc)) {
  myplclust(l_hc[[i]],
            lab.col = dgl_NOIs$colours, #x1$colours,pal1$value
            labels = dgl_NOIs$samples$group,#pal1$group
       xlab = str_glue("{names(l_hc[i])}"),
       main = str_glue("{enframe(l_hc[[i]])$value[7]} - {enframe(l_hc[[i]])$value[5]} - group"))
  legend("topright",
       legend = levels(as_factor(dgl_NOIs$samples$group)),
       fill = levels(as_factor(dgl_NOIs$colours)), #x1$colours  
       bty="n",
       cex = 0.9)
}
...
```{r}
# we use voomTMM object
voomQW_norm %>% names

vfit <- lmFit(voomQW_norm[[2]], design = design)
vfit <- contrasts.fit(vfit, coefficients = ncol(design))
...
#IPP_C_vs_noAb
topIPP_C_vs_noAb_voom <- topTable(efit,
                                  coef = 1,
                             number = nrow(efit),
                             adjust.method = "fdr",
                             sort.by = "p") %>% 
