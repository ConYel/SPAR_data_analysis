...
```{r}
# load dataset ----
DT <- fread(list.files(path = dat_path, pattern = "raw" ))
...
hist(res_AD_vs_Control$pvalue[res_AD_vs_Control$baseMean > 1], 
     breaks = 0:20/20,
...
x1_TMMwsp <- estimateDisp(edger_norm[[4]], design=design, robust=TRUE)
plotBCV(x1_TMMwsp)
fit <- glmQLFit(x1_TMMwsp, design, robust = TRUE)
...
topedgeR_res_AD_vs_Control <- topTags(edgeR_res_AD_vs_Control, 
  ...
  write_tsv(str_glue("{dat_path}/edgeR_res_AD_vs_Ctl_{date}_{my_exp}.txt"))


# voom   ----
vfit <- lmFit(voomQW_norm[[1]], design=design)
...
toplvoom_res_AD_vs_Control  <- topTable(vfit,
                             ...
                             sort.by = "p") %>% 
  as_tibble(rownames = "smallRNA") %>% 
  write_tsv(str_glue("{dat_path}/voom_res_AD_vs_Ctl_{todate}_{my_exp}.txt"))
